...
const CategoryChip = ({ label, isActive, onClick }) => {
  return (
    <button
      ...
      style={{
        backgroundColor: isActive ? "#111827" : "#f3f4f6", // gray-900 or gray-100
        color: isActive ? "#ffffff" : "#374151", // white or gray-700
        borderWidth: "1px",
...
const FiltersHeader = ({ filters, setFilters }) => {
  ...
  return (
    <div className="sticky top-0 z-10 bg-white/90 backdrop-blur px-4 py-3 border-b border-gray-200/60 space-y-4">
      {/* Title */}
        ...
        <div className="pt-2 border-t border-gray-200/40 space-y-1">
          {filters.city && (
            <div className="text-xs text-gray-600 text-right">
              🏙️ شهر:{" "}
              <span className="font-medium text-gray-900">{filters.city}</span>
            </div>
          )}
          {filters.priceRange &&
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        <div className="relative">
          <button
            ...
            className="w-full px-3 py-2.5 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors duration-150 text-right focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary-500"
          >
          ...
          {showMorePopover && (
            <div className="absolute top-full right-0 mt-2 w-full bg-white border border-gray-200 rounded-xl shadow-lg z-50 p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 justify-end">
                {materials.map((material) => (
                  <button
                    key={material}
                    className="px-2.5 py-1 text-xs font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-full border border-gray-200 transition-colors duration-150"
                    onClick={() => {
        ...
        <button
          ...
          className="w-full px-4 py-2.5 text-sm font-semibold text-gray-700 bg-white border-2 border-gray-300 rounded-lg hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 text-right"
        >
