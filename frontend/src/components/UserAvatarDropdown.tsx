...
const UserAvatarDropdown: React.FC<UserAvatarDropdownProps> = ({ user }) => {
  ...
  const navigateToCreatorDashboard = () => navigate("/creator/dashboard");
  const navigateToSellerDashboard = () => navigate("/seller");
  const navigateToAdminDashboard = () => navigate("/admin");
  const navigateToMyOrders = () => navigate("/store/orders");
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="py-1">
            ...
            {/* ثبت آگهی جدید */}
            <button
              onClick={() => handleMenuClick(navigateToCreatePost)}
              className="w-full text-right px-4 py-3 flex items-center gap-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
            >
              <svg
                className="w-5 h-5 flex-shrink-0"
            ...
            {/* سفارش‌های من */}
            <button
              onClick={() => handleMenuClick(navigateToMyOrders)}
              className="w-full text-right px-4 py-3 flex items-center gap-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
            >
              <svg
                className="w-5 h-5 flex-shrink-0"
            ...
            {/* داشبورد سوپر ادمین - Only for super_admin */}
            ...
            {/* داشبورد کریتور - Only for creator */}
            {user.role === "creator" && (
              <button
                onClick={() => handleMenuClick(navigateToCreatorDashboard)}
                className="w-full text-right px-4 py-3 flex items-center gap-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
              >
                <svg
                  className="w-5 h-5 flex-shrink-0"
            ...
            {/* داشبورد فروشنده - Only for seller */}
            {user.role === "seller" && (
              <button
                onClick={() => handleMenuClick(navigateToSellerDashboard)}
                className="w-full text-right px-4 py-3 flex items-center gap-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
              >
                <svg
                  className="w-5 h-5 flex-shrink-0"
